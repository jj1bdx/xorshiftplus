...
void next0() { 
	...
	s1 ^= s1 << A;
#ifdef ADD
	y = REV( ( s[ p ] = ( s1 ^ s0 ^ ( s1 >> B ) ^ ( s0 >> C ) ) ) + s0 );
#else
	y = REV( ( s[ p ] = ( s1 ^ s0 ^ ( s1 >> B ) ^ ( s0 >> C ) ) ) * M );
#endif
}
...
int main(int argc, char *argv[] ) {
	if ( argc != 64 + 5 ) {
		fprintf(stderr, "%s 0 A B C SEED (64 64-bit integers)\n", argv[0] );
		exit(1);
	}
	...
	for( int i = 0; i < 64; i++ ) s[ i ] = strtoull_check( argv[ 5 + i ] );
